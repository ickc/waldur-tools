...
That is enforced on every request rather than only arranged at the start.
`WaldurClient` is built for one origin and checks each URL against it before
attaching the `Authorization` header — including the *absolute* URLs the portal
names in its `Link: rel=next` headers, which the paginated walk follows and
which the server, not this code, chooses. A remembered token is stored with the
origin it was read for and is dropped rather than reused when the URL about to
be read is a different one; a token remembered by an older build, which has no
origin beside it, is a re-paste for the same reason.

The extension's permissions are not what makes this hold.
`optional_host_permissions` covers every https host so the extension can be
pointed at a second Waldur, so a reader who has granted that for their own
deployment has granted it broadly. The origin check is the guard.
...
Held in a variable, sent to one origin and to nothing else. On the ordinary path
...
puts it — with the origin it belongs to — in `chrome.storage.session`, which
lives in memory and is dropped on browser exit; never `localStorage`, which
would leave a token on disk long after the portal expired it. **Portal tokens
expire within the hour.** *Clear cached data* removes both the token and the
cached months.

**A portal token is not read-only.** It is a personal access token carrying the
scopes and roles of the account that issued it, so what it can do is whatever
that account can do; this extension only reads, but that is a property of this
code. What bounds the exposure is the hour of life and the single origin.
...
tests/*.test.mjs       parity with the Python, the branches the fixture does
                       not reach, the paging guards, the portal inference,
                       the figure rules
...
vendor/plotly.LICENSE  plotly's MIT terms, written beside it, not committed
```
...
minified code, reproducible in one command. `pixi run web-vendor` also writes
`vendor/plotly.LICENSE` next to it — the bundle names the MIT licence but does
not carry its text, and the release zip is a redistribution that has to.
